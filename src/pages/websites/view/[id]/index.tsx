...
import { getWebsiteById } from 'apiSdk/websites';
import { WebsiteInterface } from 'interfaces/website';
import { ScrapedDataListPage } from 'pages/scraped-data';

function WebsiteViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<WebsiteInterface>(
    () => (id ? `/websites/${id}` : null),
    () =>
      getWebsiteById(id, {
        relations: ['client'],
        ...
        {isLoading ? (
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                <FormListItem label="Url" text={data?.url} />
