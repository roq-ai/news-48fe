...
export function ClientListPage(props: ClientListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getClients({
        relations: ['user', 'topic.count', 'website.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['description.contains', 'image.contains', 'name.contains'],
        ...(params.filters || {}),
      }),
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('topic', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Topic',
          accessorKey: 'topic',
          cell: ({ row: { original: record } }: any) => record?._count?.topic,
        }
      : null,
    hasAccess('website', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Website',
          accessorKey: 'website',
          cell: ({ row: { original: record } }: any) => record?._count?.website,
        }
      : null,
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        ...
        {hasAccess('client', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/clients/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
