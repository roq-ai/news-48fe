...
type ColumnType = ColumnDef<TopicInterface, unknown>;
...
export function TopicListPage(props: TopicListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<TopicInterface>>(
    () => `/topics?params=${JSON.stringify(params)}`,
    fetcher,
  );
...

    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/topics/view/${record.id}`} passHref legacyBehavior>
            <Button
